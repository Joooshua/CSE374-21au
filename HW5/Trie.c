...
int main(int argc, char *argv[]) {
  TrieNode *root = makeNode(NULL);
  insertTrie(root, "hello\0");
  insertTrie(root, "gello\0");
  char* result = searchTrie(root, "43556\n");
  printf("%s \n", result);
  result = searchTrie(root, "43556#\n");
  printf("%s \n", result);
  
  freeTrie(root);
  //int result = wordToDigit("hello\n");
  //printf("%d \n", result);
  return EXIT_SUCCESS;
}