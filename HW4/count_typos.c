...
int main(int argc, char* argv[]) {
  ...
  int filename_length = strlen(input_filename) + strlen(TYPOS_SUFFIX) + 1;
  char* typos_filename = (char*) malloc(filename_length);
  // strcpy(typos_filename, input_filename);
  // strcat(typos_filename, TYPOS_SUFFIX);
  snprintf(typos_filename, sizeof(input_filename)+1, input_filename);
  int len = sizeof(TYPOS_SUFFIX);
  snprintf(typos_filename + sizeof(input_filename), len, TYPOS_SUFFIX);
